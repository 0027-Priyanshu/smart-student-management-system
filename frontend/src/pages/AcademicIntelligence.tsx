...
export default function AcademicIntelligence() {
  ...
  useEffect(() => {
    async function loadAtRisk() {
      ...
      try {
        ...
        const list = res.data.atRiskStudents || [];
        setAtRiskList(list);

        setOverviewMetrics({
          totalAtRisk: list.length,
          lowAttendanceCount: list.filter((s: any) => s.attendance < 75).length,
          lowGpaCount: list.filter((s: any) => s.gpa < 2.5).length,
          totalStudents: students.length || 50
        });
      } catch (err) {
  ...
  const handleAnalyzeStudent = async (studentId: string) => {
    ...
    try {
      ...
      setTrendData(sumRes.data.trendData || []);
  ...
  const loadInstitutionInsights = async () => {
    ...
    try {
      ...
      setInstituteChartData(res.data.chartData || []);
    } catch (err) {
      console.error(err);
  ...
  const handleDraftParentEmail = async () => {
    ...
    try {
      ...
      setParentEmailDraft(res.data.content);
  ...
  return (
    <DashboardShell title="Academic Intelligence Workspace">
      <div className="space-y-6">
        ...
        <div className="p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white rounded-3xl shadow-card flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border border-slate-800">
          <div className="flex items-center gap-3.5">
            <div className="p-3 bg-[#f97316] rounded-2xl text-white shadow-glow">
              <BrainCircuit size={28} />
            </div>
            <div>
              <h2 className="font-title font-black text-lg md:text-xl flex items-center gap-2">
                Academic Intelligence Workspace
                <span className="px-2.5 py-0.5 rounded-full text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 font-mono">
                  ML Risk Analytics Active
                </span>
              </h2>
              <p className="text-xs text-slate-300 font-medium mt-0.5">
                Comprehensive academic analytics, student risk diagnostic reports, performance trend tracking, and institutional insights.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white text-xs font-extrabold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer border border-white/10"
            >
              <FileText size={16} />
        ...
        {/* 5 Navigation Tabs */}
        <div className="flex border-b border-slate-200 bg-white p-1.5 rounded-2xl shadow-card gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none">
          <button
            onClick={() => handleTabChange('overview')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 cursor-pointer ${activeTab === 'overview' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            <BarChart2 size={16} className={activeTab === 'overview' ? 'text-[#f97316]' : ''} />
            <span>1. Executive Overview</span>
          </button>

          <button
            onClick={() => handleTabChange('at-risk')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 cursor-pointer ${activeTab === 'at-risk' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            <ShieldAlert size={16} className={activeTab === 'at-risk' ? 'text-[#f97316]' : ''} />
            <span>2. Students at Risk</span>
            {overviewMetrics.totalAtRisk > 0 && (
              <span className="px-2 py-0.5 rounded-full text-[10px] bg-red-500 text-white font-mono">
                {overviewMetrics.totalAtRisk}
              </span>
            )}
          </button>
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 cursor-pointer ${activeTab === 'performance' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            <TrendingUp size={16} className={activeTab === 'performance' ? 'text-[#f97316]' : ''} />
            <span>3. Performance Analyzer</span>
          </button>

          <button
            onClick={() => handleTabChange('insights')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 cursor-pointer ${activeTab === 'insights' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            <Sparkles size={16} className={activeTab === 'insights' ? 'text-[#f97316]' : ''} />
            <span>4. Institution Insights</span>
          </button>

          <button
            onClick={() => handleTabChange('reports')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 cursor-pointer ${activeTab === 'reports' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            <Download size={16} className={activeTab === 'reports' ? 'text-[#f97316]' : ''} />
            <span>5. Report Exports</span>
          </button>
        </div>
          ...
          <div className="space-y-6 animate-fadeIn">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-5 bg-white border border-slate-200 rounded-3xl shadow-card space-y-2">
                ...
                <p className="text-[11px] text-slate-500 font-medium">Require immediate academic intervention</p>
              </div>
              ...
              <div className="p-5 bg-white border border-slate-200 rounded-3xl shadow-card space-y-2">
                ...
                <p className="text-[11px] text-slate-500 font-medium">Enrolled across 4 departments</p>
              </div>
            ...
            <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-card space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">Inspect 5-semester GPA trends and subject-wise scorecards.</p>
                </button>
                ...
                >
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">Generate 6-month aggregate GPA and attendance distribution trends.</p>
                </button>
              </div>
            </div>
          </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-200 text-xs font-medium text-slate-800">
                      {atRiskList.map((student: any) => (
                        <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                          <td className="py-3.5 px-4">
                            <div className="font-bold text-slate-900">{student.name}</div>
                            <div className="text-[10px] font-mono text-slate-500">{student.enrollmentNo}</div>
                          </td>
                          <td className="py-3.5 px-4">
                            <div>{student.department || 'Computer Science'}</div>
                            <div className="text-[10px] text-slate-500">{student.semester || 'Semester 6'}</div>
                          </td>
                          <td className="py-3.5 px-4 font-mono font-bold text-slate-900">
                            {student.gpa.toFixed(2)} / 4.00
                          </td>
                          <td className="py-3.5 px-4 font-mono font-bold">
                            <span className={student.attendance < 75 ? 'text-red-600' : 'text-slate-900'}>
                              {student.attendance.toFixed(1)}%
                            </span>
                          </td>
                          <td className="py-3.5 px-4">
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold ${student.riskLevel === 'High' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                              {student.riskLevel === 'High' ? '🔴 High Risk' : '🟡 Medium Risk'} ({student.riskScore}%)
                            </span>
                          </td>
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            {profilerLoading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Right: 5-Semester GPA Trend Chart */}
