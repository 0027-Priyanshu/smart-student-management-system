...
async function runE2EAudit() {
  ...
  const auditResults: { name: string; success: boolean; details?: string }[] = [];
  ...
  // 1. LOGIN TESTS
  try {
    const adminRes = await axios.post(`${API_BASE}/auth/login`, { email: 'admin@sms.com', password: 'admin123' });
  ...
  try {
    const facultyRes = await axios.post(`${API_BASE}/auth/login`, { email: 'faculty@sms.com', password: 'faculty123' });
  ...
  try {
    const studentRes = await axios.post(`${API_BASE}/auth/login`, { email: 'student@sms.com', password: 'student123' });
  ...
  // 2. AUTH ME ENDPOINTS
  try {
    await axios.get(`${API_BASE}/auth/me`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/auth/me`, { headers: studentHeaders });
  ...
  // 3. DASHBOARD METRICS
  try {
    await axios.get(`${API_BASE}/dashboard/stats`, { headers: adminHeaders });
  ...
  try {
    const res = await axios.get(`${API_BASE}/students`, { headers: adminHeaders });
  ...
  if (createdStudentId) {
    try {
      await axios.get(`${API_BASE}/students/${createdStudentId}`, { headers: adminHeaders });
    ...
    try {
      await axios.put(`${API_BASE}/students/${createdStudentId}`, { grade: 'Senior Honor' }, { headers: adminHeaders });
    ...
    try {
      await axios.put(`${API_BASE}/students/${createdStudentId}/password`, { password: 'newStudentPass123' }, { headers: adminHeaders });
    ...
    try {
      await axios.delete(`${API_BASE}/students/${createdStudentId}`, { headers: adminHeaders });
    ...
    try {
      await axios.post(`${API_BASE}/students/${createdStudentId}/restore`, {}, { headers: adminHeaders });
  ...
  // Exports
  try {
    await axios.get(`${API_BASE}/students/export/csv`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/students/export/excel`, { headers: adminHeaders, responseType: 'arraybuffer' });
  ...
  try {
    await axios.get(`${API_BASE}/students/export/pdf`, { headers: adminHeaders, responseType: 'arraybuffer' });
  ...
  let createdCourseId = '';
  try {
    const res = await axios.get(`${API_BASE}/courses`, { headers: adminHeaders });
  ...
  if (createdCourseId) {
    try {
      await axios.put(`${API_BASE}/courses/${createdCourseId}`, { name: 'Audit Data Science 101 Advanced' }, { headers: adminHeaders });
  ...
  // 6. FACULTY API
  try {
    const res = await axios.get(`${API_BASE}/faculty`, { headers: adminHeaders });
  ...
  // 7. ATTENDANCE API
  try {
    const res = await axios.get(`${API_BASE}/attendance`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/attendance/heatmap`, { headers: adminHeaders });
  ...
  // 8. RESULTS API
  try {
    const res = await axios.get(`${API_BASE}/results`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/ai/academic-insights`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/ai/at-risk-students`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/ai/suggested-prompts`, { headers: adminHeaders });
  ...
  try {
    await axios.get(`${API_BASE}/ai/chat-history`, { headers: adminHeaders });
  ...
  try {
    await axios.post(`${API_BASE}/ai/chat`, { message: 'How many students are enrolled?' }, { headers: adminHeaders });
  ...
  try {
    await axios.post(`${API_BASE}/ai/nl-search`, { query: 'students in CSE' }, { headers: adminHeaders });
