...
export class AcademicMetricsService {
   ...
   */
  static async getStructuredInstitutionalMetrics(): Promise<any> {
    const { students } = await RepoService.findStudents({ isDeleted: false }, 1, 2000);
    const faculties = await RepoService.findFaculties({ isDeleted: false });
    const courses = await RepoService.findCourses({ isDeleted: false });
    ...
    const deptGpaSums: { [dept: string]: { sum: number; count: number } } = {};
    ...
    for (const student of students) {
      ...
      if (risk.gpa !== null) {
        totalGpaSum += risk.gpa;
        gpaSampleSize++;
        if (!deptGpaSums[sDept]) deptGpaSums[sDept] = { sum: 0, count: 0 };
   ...
   */
  static async getStructuredFacultyMetrics(facultyUserId: string): Promise<any> {
    const facultyProfile = await RepoService.findFacultyByUserId(facultyUserId);
    if (!facultyProfile) {
      return {
        generatedAt: new Date().toISOString(),
    ...
    const { students: allStudents } = await RepoService.findStudents({ isDeleted: false }, 1, 2000);
    const facultyStudents = allStudents.filter((s: any) => {
      const studentCourses = (s.enrolledCourses || []).map((c: any) => (c._id || c.id || c).toString());
      return studentCourses.some((cid: string) => courseIds.includes(cid));
    });
    ...
    return {
      ...
      departmentDistribution: [],
      courseDistribution: assignedCourses.map((c: any) => ({
        id: c._id || c.id || c,
        name: c.name || 'Course',
        code: c.code || 'CODE',
        enrolledCount: facultyStudents.filter((s: any) => (s.enrolledCourses || []).some((ec: any) => (ec._id || ec.id || ec).toString() === (c._id || c.id || c).toString())).length
      })),
      atRiskList
    };
  }
