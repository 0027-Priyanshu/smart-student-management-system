...
const seedDemoData = async () => {
    ...
    const existingDemoUsers = await User_1.default.find({ email: { $in: allDemoEmails } });
    const existingDemoUserIds = existingDemoUsers.map(u => u._id);
    if (existingDemoUserIds.length > 0) {
        const demoStudents = await Student_1.default.find({ userId: { $in: existingDemoUserIds } });
        const demoStudentIds = demoStudents.map(s => s._id);
        await Attendance_1.default.deleteMany({ studentId: { $in: demoStudentIds } });
        await Result_1.default.deleteMany({ studentId: { $in: demoStudentIds } });
        await Student_1.default.deleteMany({ userId: { $in: existingDemoUserIds } });
        await Faculty_1.default.deleteMany({ userId: { $in: existingDemoUserIds } });
        // Explicitly delete courses created by the demo seed to avoid orphans
        await Course_1.default.deleteMany({ name: { $in: ['Machine Learning for Intelligent Systems', 'Natural Language Processing', 'Mobile Application Development', 'Cloud Computing', 'Database Management Systems', 'Data Structures and Algorithms'] } });
        await User_1.default.deleteMany({ _id: { $in: existingDemoUserIds } });
    }
    const preservedStudent = await Student_1.default.findOne({ enrollmentNo: 'ENR27037739' });
    ...
    for (const cData of courseData) {
        ...
        courses.push(course);
        await Faculty_1.default.findByIdAndUpdate(facultyRecords[cData.facultyIndex]._id, {
    ...
    for (const student of students) {
        ...
        if (student.enrollmentNo.includes('0001') || student.enrollmentNo.includes('0005'))
            attendanceProbability = 0.95;
        if (student.enrollmentNo.includes('0003') || student.enrollmentNo.includes('0007'))
            attendanceProbability = 0.72;
        if (student.enrollmentNo.includes('0009') || student.enrollmentNo.includes('0011'))
            attendanceProbability = 0.60;
    ...
    for (const student of students) {
        const enrolledCourseIds = student.enrolledCourses;
        let baseGPA = 3.0;
        if (student.enrollmentNo.includes('0002') || student.enrollmentNo.includes('0005'))
            baseGPA = 3.8;
        if (student.enrollmentNo.includes('0004') || student.enrollmentNo.includes('0007'))
            baseGPA = 2.2;
        if (student.enrollmentNo.includes('0009'))
            baseGPA = 1.8;
        for (const courseId of enrolledCourseIds) {
