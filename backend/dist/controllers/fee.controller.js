...
function computeFeeStatus(paidFee, totalFee = 95000, dueDateStr = '2026-08-15') {
...
class FeeController {
    ...
    static async payStudentFees(req, res, next) {
        try {
            const requester = req.user;
            ...
            await repo_service_1.RepoService.updateStudent(student._id || student.id, {
                feesPaid: newPaid,
                feeStatus: newStatus,
    ...
    static async adminUpdateFee(req, res, next) {
        try {
            ...
            if (action === 'mark_status') {
                ...
                const targetTxn = history.find((h) => h.id === paymentId);
                if (targetTxn) {
                    currentPaid = Math.max(0, currentPaid - targetTxn.amount);
                    history = history.filter((h) => h.id !== paymentId);
                }
            }
            ...
            // Update Database
            await repo_service_1.RepoService.updateStudent(studentId, {
    ...
    static async getAllFeesAdmin(req, res, next) {
        try {
            const { students } = await repo_service_1.RepoService.findStudents({}, 1, 300);
            ...
            const feeRegistry = students.map((s) => {
    ...
    static async sendDuesReminder(req, res, next) {
        try {
            ...
            const lastReminder = globalReminderLogs.find(r => r.studentId === studentId && (nowMs - new Date(r.sentAt).getTime()) < 24 * 60 * 60 * 1000);
            if (lastReminder) {
            ...
            const deliveryReport = await notification_service_1.NotificationService.dispatchReminder({
                studentName: student.name,
                enrollmentNo: student.enrollmentNo,
            ...
            const newLog = {
                ...
                status: (deliveryReport.emailStatus === 'Delivered' || deliveryReport.smsStatus === 'Delivered') ? 'Delivered' : 'Failed',
